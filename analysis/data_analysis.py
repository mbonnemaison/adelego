...
a = first_time.plot.scatter(x = "Date", y = "Time_float", legend = False, figsize = (10,7))
...
b = time_kitchen.plot.scatter(x = "day_list", y = "time_spent_float", legend = False, figsize = (10,7))
...
#Get the dataframe ready for analysis
data = pd.read_csv('Adéle_du_2021-04-1_au_2021-04-1.csv', sep = ';')
...
presence_day = data.loc[(data['Parameter'] == 'PRESENCE')]

#============================================================================================================
#Get the first time we enter the kitchen expressed in Timedelta (24h data - variable called 'initial')
initial = presence_day.iloc[0,0] - presence_day.iloc[0,0].normalize()
#Figure out if initial is in the proper range (variable from the 3 months analysis)
if (first_time_mean - first_time_std) <= initial <= (first_time_mean + first_time_std):
    initial_in_range = 'Normal'
elif initial < (first_time_mean - first_time_std):
    initial_in_range = 'Early'
else:
    initial_in_range = 'Late'
#============================================================================================================
#Time spent in the kitchen (24h data - variable called 'duration')
timedeltas_24h = []
duration = pd.Timedelta(days = 0)
n = presence_day.index.min()
while n < presence_day.index.max():
    if presence_day['Value'].iloc[0,] == 0:
        timedeltas_24h.append(presence_day['Date'].iloc[0,]-presence_day['Date'].iloc[0,].normalize())
    timedeltas_24h.append(presence_day['Date'][n+1] - presence_day['Date'][n])
    n += 2
for i in timedeltas_24h:
    duration += i

#Figure out if duration is in the proper range (variable from the 3 months analysis)
if (time_kitchen_mean - time_kitchen_std) < duration < (time_kitchen_mean + time_kitchen_std):
    duration_in_range = 'Normal'
elif duration < (first_time_mean - first_time_std):
    duration_in_range = 'Low'
else:
    duration_in_range = 'High'

#====================================================================================================================
#Generate new history with upcoming data
day_to_day = pd.DataFrame()
today_info = pd.Series([presence_day.iloc[0,0].date(), str(initial)[7:], initial_in_range, str(duration)[7:], duration_in_range],
                       index = ['Date','First time', 'First time range', 'Time spent', 'Time spent range'], name = 0)
day_to_day = day_to_day.append(today_info)
next_day = presence_day.iloc[0,0].date() + pd.Timedelta(days = 1)
ind = 1

while next_day < pd.to_datetime('2021-04-09'):
    next_day = datetime.strftime(next_day, '%Y-%m-%-d')
    next_data = pd.read_csv(f'Adéle_du_{next_day}_au_{next_day}.csv', sep = ';')
    ...
    next_day = presence_next_day.iloc[0, 0].date() + pd.Timedelta(days=1)
...
while next_day < pd.to_datetime('2021-04-18').date():
    next_day = datetime.strftime(next_day, '%Y-%m-%-d')
    if next_day == '2021-04-14':
        next_data = pd.read_csv(f'Adéle_{next_day}_00h00.csv', sep = '\t')
    else:
        next_data = pd.read_csv(f'Adéle_{next_day}_00h00.csv', sep = ';')
    next_data = next_data.drop(['Objet', "Équipement", "Commande"], axis=1)
    next_data = next_data.rename(columns={'Type Générique': 'Parameter', 'Valeur': 'Value', 'Unité': 'Unit'})
